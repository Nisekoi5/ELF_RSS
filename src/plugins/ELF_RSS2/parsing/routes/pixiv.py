...
# 处理来源
@ParsingBase.append_handler(parsing_type="title", rex="/pixiv/")
async def handle_title(item: Dict[str, Any]) -> str:
    source = item["link"]
    # 获取 pixiv id
    if match := re.search(r"(?<=artworks/)(\d*)", source):
        ctx_state.id = match.group(1)
    else:
        ctx_state.id = "error"
